...
export const Dashboard: React.FC<DashboardProps> = ({ onNavigate }) => {
  ...
  return (
    <div className="relative min-h-screen bg-[#ececeb] text-[#222222] font-sans pb-16">
      ...
      {/* COMPLETE INTELLIGENT STAGE (Masthead + 3 Rigid Glass Metric Cards + Unified Stage Children) */}
      <IntelligentStage
      ...
      >
        {/* EXECUTIVE COMMAND BAR & PUBLISHING LIFECYCLE */}
        <section className="stage-glass-panel p-6 sm:p-8" aria-label="Executive Command Center">
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
            <div className="space-y-2.5 max-w-2xl">
              <div className="flex flex-wrap items-center gap-2.5">
                <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-emerald-500/10 border border-emerald-500/25 text-emerald-800 text-xs font-mono font-semibold">
                  <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" />
                  All Systems Operational · 98% Health
                </span>

                <span className="px-3 py-1 rounded-full bg-black/5 border border-black/8 text-[#44444a] text-xs font-mono font-medium">
                  Theme {activeManifest.number}: {activeManifest.name} (23 Themes)
                </span>
              </div>
              ...
              <p className="text-xs sm:text-sm text-[#4a4a4a] leading-relaxed">
                Real-time portfolio telemetry, 23 structurally isolated theme architectures, and autonomous social distribution engine.
              </p>
            </div>

            {/* Spec-Accurate Action Pills */}
            <div className="flex flex-wrap items-center gap-2.5">
              <button
                onClick={() => onNavigate('/admin/projects/new')}
                data-testid="dash-new-project-btn"
                className="stage-pill-action stage-pill-action-crimson"
              >
              ...
              <button
                ...
                className="stage-pill-action stage-pill-action-dark"
              >
              ...
              <button
                ...
                className="stage-pill-action stage-pill-action-glass"
              >
                <Eye className="w-4 h-4 text-purple-600" /> Visual Site Editor
              </button>
              ...
              <button
                ...
                className="stage-pill-action stage-pill-action-glass"
              >
                <Users className="w-4 h-4 text-emerald-600" /> Recruiter Mode
              </button>
            </div>
          </div>

          {/* Publishing Lifecycle Bar */}
          <div className="mt-6 pt-5 border-t border-black/8 flex flex-wrap items-center justify-between gap-4 text-xs font-mono text-[#55555e]">
            <div className="flex items-center gap-4">
              <div className="flex items-center gap-2">
                <span className="text-[#888890]">STATE:</span>
                <span className={`px-2.5 py-0.5 rounded-full font-bold uppercase text-[11px] ${
                  siteMode === 'live' ? 'bg-emerald-500/15 text-emerald-800' : 'bg-amber-500/15 text-amber-800'
                }`}>
                  ● {siteMode}
                </span>
              </div>
              <div className="hidden sm:block text-black/15">|</div>
              <div className="flex items-center gap-2">
                <span className="text-[#888890]">ACTIVE THEME:</span>
                <span className="text-[#1a1a1a] font-bold">{activeManifest.name}</span>
              </div>
              <div className="hidden sm:block text-black/15">|</div>
              <div className="flex items-center gap-2">
                <span className="text-[#888890]">LAST PUBLISHED:</span>
                <span className="text-[#2d2d2d]">Today, 22:43 UTC</span>
              </div>
            </div>
            ...
            {siteMode === 'draft' && (
              <button
                ...
                className="stage-pill-action bg-emerald-600 hover:bg-emerald-700 text-white shadow-md text-xs font-semibold"
              >
        ...
        {/* VITAL SIGNALS BENTO GRID (4 Frosted Glass Metric Tiles) */}
        <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4" aria-label="Vital Signals">
          {/* Metric 1: Site Health & Speed */}
          <div 
            onClick={() => onNavigate('/admin/site-health')}
            className="stage-glass-panel p-5 flex flex-col justify-between cursor-pointer group"
          >
            <div>
              <div className="flex items-center justify-between mb-3">
                <span className="text-xs font-mono text-[#666670] uppercase tracking-wider font-semibold">Health & Speed</span>
                <div className="w-8 h-8 rounded-xl bg-emerald-500/15 text-emerald-700 flex items-center justify-center">
                  <ShieldCheck className="w-4 h-4" />
                </div>
              </div>
              <div className="flex items-baseline gap-2">
                <span className="text-3xl font-black text-[#1a1a1a] tracking-tight">98%</span>
                <span className="text-xs font-mono text-emerald-700 font-bold">Optimal</span>
              </div>
            </div>
            <div className="pt-4 border-t border-black/8 mt-4 flex items-center justify-between text-[11px] font-mono text-[#55555e]">
              <span>LCP: 0.8s</span>
              <span>CLS: 0.01</span>
              <span>SEO: 99</span>
            </div>
          ...
          >
            <div>
              <div className="flex items-center justify-between mb-3">
                <span className="text-xs font-mono text-[#666670] uppercase tracking-wider font-semibold">Production Systems</span>
                <div className="w-8 h-8 rounded-xl bg-blue-500/15 text-blue-700 flex items-center justify-center">
                  <FolderGit2 className="w-4 h-4" />
                </div>
              </div>
              <div className="flex items-baseline gap-2">
                <span className="text-3xl font-black text-[#1a1a1a] tracking-tight">{projects.length}</span>
                <span className="text-xs font-mono text-blue-700 font-bold">Deployed</span>
              </div>
            </div>
            <div className="pt-4 border-t border-black/8 mt-4 flex items-center justify-between text-[11px] text-[#55555e]">
              <span>{projects.filter(p => p.featured).length} Featured</span>
              <span className="text-blue-700 font-semibold group-hover:underline">Manage →</span>
            </div>
          </div>

          {/* Metric 3: Audience & Recruiter Telemetry */}
          <div 
          ...
          >
            <div>
              <div className="flex items-center justify-between mb-3">
                <span className="text-xs font-mono text-[#666670] uppercase tracking-wider font-semibold">Audience Telemetry</span>
                <div className="w-8 h-8 rounded-xl bg-purple-500/15 text-purple-700 flex items-center justify-center">
                  <Activity className="w-4 h-4" />
                </div>
              </div>
              <div className="flex items-baseline gap-2">
                <span className="text-3xl font-black text-[#1a1a1a] tracking-tight">4,820</span>
                <span className="text-xs font-mono text-emerald-700 flex items-center gap-0.5 font-bold">
            ...
            <div className="pt-4 border-t border-black/8 mt-4 flex items-center justify-between text-[11px] text-[#55555e]">
              ...
              <span className="text-purple-700 font-semibold group-hover:underline">Telemetry →</span>
            </div>
          </div>

          {/* Metric 4: Telegram & Social Queue */}
          <div 
          ...
          >
            <div>
              <div className="flex items-center justify-between mb-3">
                <span className="text-xs font-mono text-[#666670] uppercase tracking-wider font-semibold">Autonomous Queue</span>
                <div className="w-8 h-8 rounded-xl bg-amber-500/15 text-amber-700 flex items-center justify-center">
                  <Send className="w-4 h-4" />
                </div>
              </div>
              <div className="flex items-baseline gap-2">
                <span className="text-3xl font-black text-[#1a1a1a] tracking-tight">{pendingDrafts.length}</span>
                <span className="text-xs font-mono text-amber-800 font-bold">Needs Review</span>
              </div>
            </div>
            <div className="pt-4 border-t border-black/8 mt-4 flex items-center justify-between text-[11px] text-[#55555e]">
              <span>Autonomous Pipeline</span>
              <span className="text-amber-800 font-semibold group-hover:underline">Review →</span>
            </div>
          </div>
        </section>
